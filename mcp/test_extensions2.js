...
(async function test() {
  ...
  // 2. 先获取真实商品列表
  const { json: productsJson } = await request("GET", "/admin/products", null, token);
  const productName = productsJson[0]?.name;
  console.log("📦 真实商品:", productName ? productName : "无商品数据");
  ...
  const { json: createOrderJson, status: createOrderStatus } = await request("POST", "/admin/orders", {
    store: "天河北店",
    items: productName ? `${productName}×1` : "测试商品×1",
    total: 20,
    source: "测试脚本"
  }, token);
  ...
  if (completeOrderJson.upgrade) {
    ...
    console.log("✅ 积分变化:", (newPoints - (initialMember?.points || 0)) + " 分");
